...
export class Script
{
    ...
    public AddElement(element: HTMLDivElement): void
    {
        ...
        {
            container: element,
            text: Main.ThrowIfNullOrUndefined(element.querySelector(`.text.accuracy._01 > .accuracy`))
        };
    }
    ...
    public ResetData(): void
    {
        ...
        {
            this.elements[key].text.innerText = "Accuracy";
        }
    }
    ...
    public UpdateLiveData(data: LiveData): void
    {
        for (const key of Object.keys(this.elements))
        {
            ...
            element.text.innerText = `${Math.round(data.Accuracy * 10) / 10}%`;
        }
    }
}
