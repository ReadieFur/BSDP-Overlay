...
export class Script
{
    ...
    public AddElement(element: HTMLDivElement): void
    {
        ...
        {
            container: element,
            maxScore: Main.ThrowIfNullOrUndefined(element.querySelector(`.text.max_score._01 > .maxScore`))
        };
    }
    ...
    public UpdateStyles(element: HTMLDivElement, styles: TCustomStyles): void
    {
        ...
        {
            this.elements[element.id].maxScore.style.fontSize = `${styles.fontSize}px`;
        }
        else
        {
            this.elements[element.id].maxScore.style.removeProperty("fontSize");
        }
    }
    ...
    public UpdateLiveData(data: LiveData): void
    {
        for (const key of Object.keys(this.elements))
        {
            ...
            element.maxScore.innerText = UI.SeperateNumber(data.MaxScoreWithMultipliers);
        }
    }
}
